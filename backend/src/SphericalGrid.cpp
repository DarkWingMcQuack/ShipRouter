...
auto SphericalGrid::findNearestNodeInUpperRow(std::size_t m, std::size_t n) const noexcept
    -> std::pair<std::size_t, std::size_t>
{
    ...
    auto node = gridToID(m, n);

    auto search_row = m + 1;
    auto n_cols_in_upper = nCols(search_row);
    bool changed = true;
    std::size_t current = 0;

    while(changed) {
        auto left_n = (n - 1 + n_cols_in_upper) % n_cols_in_upper;
        auto right_n = (n + 1) % n_cols_in_upper;

        auto last = gridToID(search_row, current);
        auto left = gridToID(search_row, left_n);
        auto right = gridToID(search_row, right_n);

        if(distanceBetween(last, node) < distanceBetween(left, node)) {
            changed = true;
            current = left_n;
            continue;
        }

        if(distanceBetween(last, node) < distanceBetween(right, node)) {
            changed = true;
            current = right_n;
            continue;
        }
    ...
    return std::pair{search_row, current};
}

auto SphericalGrid::findNearestNodeInLowerRow(std::size_t m, std::size_t n) const noexcept
    -> std::pair<std::size_t, std::size_t>
{
